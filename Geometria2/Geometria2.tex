...
\begin{ex}
	...
	$Id:(\mathbb{R}, \tau_D) \rightarrow (\mathbb{R}, \tau_E)$, $Id:(\mathbb{R}, \tau_E) \rightarrow (\mathbb{R}, \tau_C)$, $Id:(\mathbb{R}, \tau_C) \rightarrow (\mathbb{R}, \tau_I)$. Nessuna delle inverse è però continua. Più in generale, $Id: (X, \tau) \rightarrow (X, \simga)$ con $\sigma \subset \atu$ è continua ma l'inversa no.
\ebd{ex}

\begin{defn}
Dato un insieme $X$ su cui sono definite le topologie $\tau$ e $\tau'$, si dice che $\tau$ \`e meno fine di $\tau'$ se $\tau \subseteq \tau'$.
\end{defn}

\begin{oss}
Con il setting della definizione sopra, si pu\`o dire che $\tau$ \`e meno fine di $\tau'$ se e solo se ${Id_X:(X,\tau')\rightarrow(X, \tau)}$ \`e continua.
\end{oss}
